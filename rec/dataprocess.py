...
def process(train_data, val_data, test_data):
    ...
    train_user_set, val_user_set, test_user_set, train_item_set = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
...
def load_data(args):
    ...
    with open(dataset_file, 'rb') as file:
        train_cf, val_cf, test_cf = pickle.load(file)
    if args.flag == 1:
        aug_edge_filename = args.dataset_path + '/aug/aug_edges_{}_{}.pkl'.format(args.strategy,  str(args.aug_num))
        with open(aug_edge_filename, 'rb') as f:
            aug_cf = pickle.load(f)
        train_cf.extend(aug_cf)
    elif args.flag == 2:
        aug_edge_filename = args.dataset_path + '/aug/aug_edges_user_{}_{}.pkl'.format(args.strategy, str(args.aug_num))
        with open(aug_edge_filename, 'rb') as f:
            aug_cf = pickle.load(f)
        train_cf.extend(aug_cf)
        
    train_cf = np.array(train_cf)
    val_cf = np.array(val_cf)
...
def normalize_edge(edge_index, n_users, n_items):
    ...
    return torch.sparse_coo_tensor(edge_index, edge_weight, (n_users + n_items, n_users + n_items)), deg